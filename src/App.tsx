...
import { UploadIcon, SparklesIcon, AlertTriangleIcon, DownloadIcon, LockClosedIcon, LockOpenIcon, CropIcon, WandIcon, BgRemoveIcon, ClearIcon } from './components/icons';
import { SAMPLE_IMAGE_URL } from './components/sampleImage';
...
const App: React.FC = () => {
  ...
  const displayImageUrl = useMemo(() => {
    ...
    if (isUsingSample) return SAMPLE_IMAGE_URL;
    return null;
  }, [originalImage, croppedImageUrl, isUsingSample]);

  const originalImageUrlForCropper = useMemo(() => {
    if (originalImage) return URL.createObjectURL(originalImage);
    if (isUsingSample && !originalImage) return SAMPLE_IMAGE_URL;
    return null;
  }, [originalImage, isUsingSample]);

  // Load sample image dimensions on initial mount
  useEffect(() => {
    if (!isUsingSample) return;
    const img = new Image();
    img.onload = () => {
  ...
  const getActiveImagePart = useCallback(async (): Promise<Part> => {
    if (croppedImageUrl) {
        return dataUrlToGenerativePart(croppedImageUrl);
    }
    if (originalImage) {
        return fileToGenerativePart(originalImage);
    }
    if (isUsingSample) {
        return dataUrlToGenerativePart(SAMPLE_IMAGE_URL);
    }
    throw new Error("No image available to process.");
  }, [croppedImageUrl, originalImage, isUsingSample]);
  ...
  const handleGenerateClick = useCallback(async () => {
    ...
    setEditedImage(null);
    try {
      const imagePart = await getActiveImagePart();
  ...
  }, [displayImageUrl, prompt, getActiveImagePart, processAndSetImage]);
  ...
  }, [displayImageUrl, getActiveImagePart, processAndSetImage]);
        ...
        <div className="w-full lg:w-1/3 flex flex-col gap-6 p-6 bg-gray-800/50 rounded-xl border border-gray-700 shadow-2xl">
          <div>
            <div className="flex justify-between items-center mb-2">
                <label className="text-xl font-semibold text-gray-200">1. Upload Image</label>
                {displayImageUrl && (
                    <button onClick={handleClearImage} className="flex items-center gap-1 text-sm text-gray-400 hover:text-red-400 transition-colors" title="Clear image and start over">
            ...
            <div className="mt-2 flex justify-center rounded-lg border-2 border-dashed border-gray-600 px-6 py-10 hover:border-blue-400 transition-colors">
              {!displayImageUrl ? (
                ...
                <div className="text-center">
                    <img src={displayImageUrl} alt="Current image preview" className="max-h-24 mx-auto mb-4 rounded-md border-2 border-gray-700" />
                    <label htmlFor="file-upload" className="relative cursor-pointer rounded-md font-semibold text-blue-400 hover:text-blue-300">
          ...
          <fieldset disabled={!displayImageUrl} className="disabled:opacity-50 transition-opacity">
          ...
          <div>
            ...
            <textarea id="prompt" rows={4} className="w-full p-3 bg-gray-700 border-2 border-gray-600 rounded-md focus:ring-2 focus:ring-blue-500 transition disabled:opacity-50" placeholder="e.g., 'Add a retro cinematic filter'" value={prompt} onChange={(e) => setPrompt(e.target.value)} disabled={!displayImageUrl} />
          </div>
          ...
          <div className="mt-auto pt-6 border-t border-gray-700/50 flex flex-col gap-4">
            <button 
              ...
              disabled={isLoading || isEnhancing || isRemovingBg || !displayImageUrl} 
            ...
            <button 
              ...
              disabled={isLoading || isEnhancing || isRemovingBg || !displayImageUrl || !prompt.trim()} 
